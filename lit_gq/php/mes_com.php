...
$rst = $dbh->query("SELECT comment.to_comment_id AS to_cid,comment.add_time AS tim,comment.say AS com,user.uname AS nam,user.headimg AS img FROM comment JOIN user ON comment.user_id=user.user_id WHERE to_comment_id IN (SELECT comment_id FROM comment WHERE user_id='$uid')");
